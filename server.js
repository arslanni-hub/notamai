    ...
    req.on('end', async () => {
      try {
        ...
        if (type === 'SIGMET') {
          ...
          analyzeSystemPrompt = 'You are an expert aviation meteorologist with complete knowledge of all world airports and their ICAO codes.\n\nCRITICAL ICAO IDENTIFICATION RULES:\n1. You have complete knowledge of ALL world airports and their ICAO codes from your training data. Use this knowledge directly - do not guess.\n2. For Turkish airports - use ONLY this verified DHMİ list (Wikipedia-sourced):\nLTAC=Ankara Esenboğa, LTAD=Ankara Etimesgut Air Base, LTAE=Ankara Mürted Air Base, LTAF=Adana Şakirpaşa, LTAG=Adana İncirlik Air Base, LTAH=Afyon, LTAI=Antalya, LTAJ=Gaziantep(old), LTAN=Konya, LTAP=Amasya Merzifon, LTAR=Sivas Nuri Demirağ, LTAS=Zonguldak Çaycuma, LTAT=Malatya Erhaç, LTAU=Kayseri Erkilet, LTAV=Kayseri(military), LTAW=Tokat, LTAY=Denizli Çardak, LTAZ=Nevşehir Kapadokya, LTBA=Istanbul Atatürk(CLOSED 2019), LTBD=Aydın Çıldır, LTBF=Balıkesir Merkez, LTBG=Bandırma Air Base, LTBH=Çanakkale, LTBI=Eskişehir Air Base, LTBJ=İzmir Adnan Menderes, LTBL=İzmir Çiğli Military, LTBN=Kahramanmaraş(old), LTBO=Uşak, LTBQ=Kocaeli Cengiz Topel, LTBR=Bursa Yenişehir, LTBS=Dalaman, LTBT=Hakkari Yüksekova(old), LTBU=Tekirdağ Çorlu, LTBV=Adıyaman(old), LTBW=Istanbul Hezarfen Airfield, LTBX=Erzincan(old), LTBY=Eskişehir Hasan Polatkan, LTBZ=Kütahya Zafer, LTCA=Elazığ, LTCB=Ordu-Giresun, LTCC=Diyarbakır, LTCD=Erzincan, LTCE=Erzurum, LTCF=Kars Harakani, LTCG=Trabzon, LTCH=Şanlıurfa GAP(old), LTCI=Van Ferit Melen, LTCJ=Batman, LTCK=Muş, LTCL=Siirt, LTCM=Sinop, LTCN=Kahramanmaraş, LTCO=Ağrı Ahmed-i Hani, LTCP=Adıyaman, LTCR=Mardin, LTCS=Şanlıurfa GAP, LTCT=Iğdır, LTCU=Bingöl, LTCV=Şırnak Şerafettin Elçi, LTCW=Hakkari Yüksekova Selahaddin Eyyubi, LTDA=Hatay, LTDB=Mersin Çukurova International, LTFD=Balıkesir Koca Seyit, LTFE=Milas-Bodrum, LTFG=Gazipaşa-Alanya, LTFH=Samsun-Çarşamba, LTFI=Karabük-Safranbolu, LTFJ=Istanbul Sabiha Gökçen, LTFK=Gökçeada, LTFL=Isparta(new), LTFM=Istanbul Airport(main), LTFN=Amasra, LTFO=Rize-Artvin, LTFB=Selçuk-Efes\nCRITICAL: LTAY=Denizli Çardak(NOT Isparta), LTFM=Istanbul Airport(NOT Atatürk-CLOSED), LTFJ=Istanbul Sabiha Gökçen(NOT İzmir), LTBJ=İzmir Adnan Menderes(NOT Istanbul), LTDB=Mersin Çukurova International\n3. For ALL other world airports, use your complete training knowledge to identify the correct official airport name and city.\n4. NEVER make up or guess airport names - if truly uncertain, just state the ICAO code.\n\nDecode this METAR and provide:\n1. Airport name and ICAO code (verified correct)\n2. Current conditions summary (wind, visibility, weather)\n3. Ceiling and cloud layers\n4. Temperature, dewpoint, pressure\n5. Any hazards or significant phenomena\n6. Operational recommendation (VFR/IFR/MVFR status)\n\nBe concise, 4-6 bullet points, practical for flight crews.';
        } else if (type === 'TAF') {
