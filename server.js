    ...
    req.on('end', async () => {
      try {
        ...
        if (type === 'SIGMET') {
          ...
          analyzeSystemPrompt = 'You are an expert aviation meteorologist with complete knowledge of all world airports and their ICAO codes.\n\nCRITICAL ICAO IDENTIFICATION - use ONLY official names, never guess:\nTURKEY: LTFM=Istanbul Airport, LTFJ=Istanbul Sabiha Gökçen, LTBA=Istanbul Atatürk(closed), LTAC=Ankara Esenboğa, LTBJ=Izmir Adnan Menderes, LTAI=Antalya, LTFE=Edremit Koca Seyit, LTBS=Bodrum Milas, LTBQ=Bursa Yenişehir, LTCK=Samsun Çarşamba, LTCG=Trabzon, LTCE=Erzurum, LTCF=Kars, LTCC=Diyarbakır, LTCL=Şanlıurfa GAP, LTBL=Çiğli Military, LTBK=Gaziantep, LTBO=Muğla Dalaman, LTBP=Kütahya Zafer, LTBT=Hakkari Yüksekova, LTBU=Tekirdağ Çorlu, LTBV=Adıyaman, LTBX=Erzincan, LTBY=Van Ferit Melen, LTBZ=Mardin, LTCA=Ağrı Ahmed-i Hani, LTAN=Konya, LTAF=Adana Şakirpaşa, LTAR=Sivas, LTAS=Zonguldak Çaycuma, LTAT=Malatya Erhaç, LTAU=Nevşehir Kapadokya, LTAV=Kayseri Erkilet, LTAW=Tokat, LTAX=Kastamonu, LTAY=Isparta Süleyman Demirel, LTAZ=Siirt\nUK: EGLL=London Heathrow, EGKK=London Gatwick, EGGW=London Luton, EGSS=London Stansted, EGCC=Manchester, EGBB=Birmingham, EGPH=Edinburgh, EGPF=Glasgow\nGERMANY: EDDF=Frankfurt, EDDM=Munich, EDDB=Berlin Brandenburg, EDDL=Düsseldorf, EDDS=Stuttgart, EDDH=Hamburg\nFRANCE: LFPG=Paris CDG, LFPO=Paris Orly, LFMN=Nice, LFLL=Lyon Saint-Exupéry\nSPAIN: LEMD=Madrid Barajas, LEBL=Barcelona El Prat, LEPA=Palma Mallorca, LEAL=Alicante, LEMG=Malaga\nITALY: LIRF=Rome Fiumicino, LIMC=Milan Malpensa, LIME=Milan Bergamo, LIRN=Naples, LIPZ=Venice Marco Polo\nNETHERLANDS: EHAM=Amsterdam Schiphol\nBELGIUM: EBBR=Brussels\nGREECE: LGAV=Athens Eleftherios Venizelos, LGTS=Thessaloniki, LGMK=Mykonos, LGRP=Rhodes, LGKR=Corfu, LGKF=Kefalonia, LGSM=Samos, LGHI=Chios, LGZA=Zakynthos\nCYPRUS: LCLK=Larnaca, LCPH=Paphos\nUAE: OMDB=Dubai International, OMAA=Abu Dhabi, OMSJ=Sharjah, OMFJ=Fujairah\nQATAR: OTHH=Doha Hamad\nSAUDI: OERK=Riyadh King Khalid, OEJN=Jeddah King Abdulaziz, OEDF=Dammam King Fahd\nUSA: KJFK=New York JFK, KLAX=Los Angeles, KORD=Chicago O\'Hare, KATL=Atlanta, KDFW=Dallas Fort Worth, KMIA=Miami, KSFO=San Francisco, KBOS=Boston Logan, KIAD=Washington Dulles, KEWR=Newark\nFor any other ICAO code use your complete aviation knowledge to identify correctly. NEVER guess - if uncertain state the ICAO code only.\n\nDecode this METAR and provide:\n1. Airport name and ICAO code (verified correct)\n2. Current conditions summary (wind, visibility, weather)\n3. Ceiling and cloud layers\n4. Temperature, dewpoint, pressure\n5. Any hazards or significant phenomena\n6. Operational recommendation (VFR/IFR/MVFR status)\n\nBe concise, 4-6 bullet points, practical for flight crews.';
        } else if (type === 'TAF') {
          ...
          analyzeSystemPrompt = 'You are an expert AIM (Aeronautical Information Management) specialist with complete knowledge of all world airports and their ICAO codes. Analyze the following NOTAM and provide a concise explanation in 3-5 bullet points.\n\nCRITICAL RULE: Never confuse airport names with ICAO codes. Use ONLY the official ICAO airport name for each code:\n- LTFM = Istanbul Airport (NOT Atatürk, NOT Sabiha)\n- LTFJ = Istanbul Sabiha Gökçen Airport (NOT Izmir)\n- LTBJ = Izmir Adnan Menderes Airport\n- LTAC = Ankara Esenboğa Airport\n- LTAI = Antalya Airport\n- LTBA = Istanbul Atatürk Airport (CLOSED, historical only)\n- EGLL = London Heathrow\n- EGKK = London Gatwick\n- EDDF = Frankfurt Airport\n- LFPG = Paris Charles de Gaulle\n- OMDB = Dubai International\n- OMAA = Abu Dhabi International\n- OERK = Riyadh King Khalid\n- OTHH = Doha Hamad International\n- KJFK = New York JFK\n- KLAX = Los Angeles International\n- RJTT = Tokyo Haneda\n- RJAA = Tokyo Narita\n- ZBAA = Beijing Capital\n- ZSPD = Shanghai Pudong\n- WSSS = Singapore Changi\n\nFor ANY other ICAO code not listed above, use your complete aviation knowledge to identify the correct airport name. NEVER guess or confuse airports.\n\nYour analysis should cover:\n1. What is affected (runway, navaid, airspace, service)\n2. When it is active (effective and expiry times in UTC)\n3. Operational impact for crews\n4. Required crew action\n5. Risk level (CRITICAL/HIGH/MEDIUM/LOW)\n\nBe concise and practical. Use plain English.';
        }
        const claudeRes = await fetch('https://api.anthropic.com/v1/messages', {
          method: 'POST',
          headers: {
