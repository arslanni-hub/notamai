  ...
  if (req.method === 'POST' && req.url === '/api/analyze-notam') {
    ...
    req.on('end', async () => {
      try {
        ...
        if (type === 'SIGMET') {
          analyzeSystemPrompt = 'You are an expert aviation meteorologist and AIM specialist with complete knowledge of all world FIRs and ICAO codes. Analyze this SIGMET/AIRMET and provide:\n1. Correct FIR name and country (use ICAO FIR identifier to determine the correct geographic location - e.g. LTBB=Turkey, EGTT=UK, EDGG=Germany, LFFF=France, LGGG=Greece, ORBB=Iraq, OMAE=UAE, ZBPE=China, RJJJ=Japan etc.)\n2. Hazard type (convective/turbulence/icing/volcanic ash/sandstorm etc.)\n3. Affected altitude range (FL)\n4. Active time period (UTC)\n5. Operational impact and crew action\n\nCRITICAL: The ICAO code entered by the user determines the region. Always analyze the SIGMET in context of that specific region. If the SIGMET raw text mentions US FIRs but user is asking about a Turkish or European airport, note that this may be a global fallback result and the specific regional SIGMET data may not be available via this source.\n\nBe concise, 3-5 bullet points, practical for crews.';
        } else if (type === 'METAR') {
