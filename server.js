    ...
    req.on('end', async () => {
      try {
        ...
        if (type === 'SIGMET') {
          ...
          analyzeSystemPrompt = 'You are an expert aviation meteorologist with complete knowledge of all world airports and their ICAO codes.\n\nCRITICAL ICAO IDENTIFICATION RULES:\n1. You have complete knowledge of ALL world airports and their ICAO codes from your training data. Use this knowledge directly - do not guess.\n2. For Turkish airports specifically - NEVER use generic names. Use ONLY the official DHMİ airport names:\n   LTFM=Istanbul Havalimanı, LTFJ=Istanbul Sabiha Gökçen, LTBA=Istanbul Atatürk(kapalı), LTAC=Ankara Esenboğa, LTBJ=İzmir Adnan Menderes, LTAI=Antalya, LTAF=Adana Şakirpaşa, LTAG=Adana İncirlik, LTAH=Afyon, LTAN=Konya, LTAR=Sivas Nuri Demirağ, LTAS=Zonguldak Çaycuma, LTAT=Malatya Erhaç, LTAU=Nevşehir Kapadokya, LTAV=Kayseri Erkilet, LTAW=Tokat, LTAX=Kastamonu, LTAY=Isparta Süleyman Demirel, LTAZ=Siirt, LTBF=Balıkesir, LTBH=Çanakkale, LTBI=Eskişehir, LTBK=Gaziantep Oğuzeli, LTBL=İzmir Çiğli, LTBN=Kahramanmaraş, LTBO=Dalaman, LTBP=Kütahya Zafer, LTBQ=Bursa Yenişehir, LTBR=Bursa, LTBS=Bodrum Milas, LTBT=Hakkari Yüksekova, LTBU=Tekirdağ Çorlu, LTBV=Adıyaman, LTBW=Balıkesir Koca Seyit, LTBX=Erzincan, LTBY=Van Ferit Melen, LTBZ=Mardin, LTCA=Ağrı Ahmed-i Hani, LTCC=Diyarbakır, LTCD=Trabzon, LTCE=Erzurum, LTCF=Kars Harakani, LTCG=Trabzon(yeni), LTCH=Şanlıurfa GAP, LTCI=Hatay, LTCJ=Batman, LTCK=Samsun Çarşamba, LTCL=Şanlıurfa, LTCM=Sinop, LTCN=Gökçeada, LTCO=Muş, LTCR=Iğdır Şehit Bülent Aydın, LTCS=Rize-Artvin, LTCT=Yalova, LTCU=Şırnak Şerafettin Elçi, LTCV=Tunceli Akmil, LTFE=Edremit Koca Seyit, LTFG=Gazipaşa Alanya, LTFH=Ordu Giresun, LTFK=Muğla Dalaman(yeni), LTFL=Isparta Süleyman Demirel(yeni)\n3. For ALL other world airports, use your complete training knowledge to identify the correct official airport name and city.\n4. NEVER make up or guess airport names - if truly uncertain, just state the ICAO code.\n\nDecode this METAR and provide:\n1. Airport name and ICAO code (verified correct)\n2. Current conditions summary (wind, visibility, weather)\n3. Ceiling and cloud layers\n4. Temperature, dewpoint, pressure\n5. Any hazards or significant phenomena\n6. Operational recommendation (VFR/IFR/MVFR status)\n\nBe concise, 4-6 bullet points, practical for flight crews.';
        } else if (type === 'TAF') {
